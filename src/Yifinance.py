...
class BilibiliTradingStrategy:
    def __init__(self):
        ...
        self.position_size = 0.5       # Increased from 0.3 to 0.5 (50% of capital per trade)
        self.stop_loss_pct = 0.05      # Reduced from 0.1 to 0.05 (5% stop loss)
        self.take_profit_pct = 0.15    # Increased from 0.1 to 0.15 (15% take profit)
        
        # Support and resistance parameters
        self.support_resistance_window = 15  # Reduced from 20 to 15 for more frequent level updates
        self.price_cluster_count = 5    
        self.support_resistance_threshold = 0.03  # Increased from 0.02 to 0.03 (3% threshold)
        self.volume_weight_factor = 0.8  # Increased from 0.7 to 0.8
        ...
        self.momentum_threshold = 0.8  # Reduced from 1.0 to 0.8 (0.8% price change as momentum)
        self.volume_threshold = 1.1    # Reduced from 1.2 to 1.1 (10% above average volume)
    ...
    def generate_signals(self):
        ...
        for i in range(3, len(self.data)):
            ...
            # Require less confirming signals (changed from 2 to 1)
            if signal >= 1:
                signal = 1
            elif signal <= -1:
                signal = -1
...
if __name__ == "__main__":
    ...
    # Download data
    start_date = '2024-11-01'  # Starting from 2023
    end_date = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')  # Yesterday
    try:
        data = strategy.download_data(start_date, end_date)
        print(f"Downloaded {len(data)} days of data for BILI")
    except Exception as e:
        print(f"Error downloading data: {e}")
        # Use a more realistic date range
    ...
    for i, trade in enumerate(strategy.all_trades):
        print(f"交易 {i+1}: {trade['date']} - {trade['action']} 价格 ${trade['price']}, {trade['shares']} 股")
        if 'reason' in trade:
            print(f"  原因: {trade['reason']}")
        if 'pnl_pct' in trade:
            print(f"  盈亏: {trade['pnl_pct']:.2f}%")
